...
## 2026-05-28
- **Phase 1 MVP Release & Security Legal Hardening**:
  * **Python 3.14 compatible line-bot-sdk imports fixed**: Resolved backend startup crashes (`ModuleNotFoundError: No module named 'linebot.v2'`) occurring due to Python 3.14 breaking changes in internal C-APIs which block source builds of old versions. Installed the modern, precompiled wheel-compatible `line-bot-sdk==3.6.0` and refactored the webhook endpoint in [/backend/app/api/v1/endpoints/line.py](file:///d:/Fillax---/backend/app/api/v1/endpoints/line.py) to use version-suffix-free imports directly (`from linebot import WebhookParser`), enabling 100% successful server boots on Port 8000.
  * **Brand Logo Asset Replaced**: Overwrote the official web logo asset `fillax-mascot.png` in the public directory with the exact purple winking glasses mascot sticking out its tongue provided by the user. Configured layout metadata to cleanly render this asset as the browser tab favicon and application header icon, maintaining 100% brand consistency.
  * **Interactive LIFF Review Webview Deployed**: Developed the high-fidelity double-pane mobile page at [/liff](file:///d:/Fillax---/frontend/src/app/liff/page.tsx). Features smooth layout split (stacked on mobile), stateful receipt image preview with Supabase storage paths, editable form inputs (Vendor, Amount, Date, Category, notes), a dynamic modulo-11 client-side DBD juristic verification badge (`DBD VERIFIED 🟢` with auto-name matching), and an HTML5 canvas signature pad enabling electronic touch-drawn signing.
  * **Legal Technology Terms of Service (TOS) & Disclaimers Deployed**: Crafted a detailed Legal Tech-compliant contract at [/tos](file:///d:/Fillax---/frontend/src/app/tos/page.tsx) to protect solo developers. Sets a strict liability limitation ceiling equal to 1 month of service fee (or 0 baht on free plan) and integrates golden disclaimer warnings across all financial interfaces.
  * **Next.js Suspense Compilation Optimization**: Refactored `/liff` searches and query parameter loading to reside within `<Suspense>` boundaries. This cleanly resolves the static CSR bailout, enabling 100% compilation health and successful optimized production builds (`npm run build` completes successfully).
